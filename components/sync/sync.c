...
_Noreturn void sync_task(void *pvParameters) {
    ...
    while (true) {
        vTaskDelay(pdMS_TO_TICKS(CONFIG_SYNC_PERIOD * 60 * 1000));
        ...
        if (xSemaphoreTake(data_mutex, portMAX_DELAY) == pdTRUE) {
            ...
            pm10 = shared_data.pm10;
            pm25 = shared_data.pm25;

            xSemaphoreGive(data_mutex);

            publish(&client, 0x01, 0x01, temperature);
            publish(&client, 0x01, 0x02, humidity);
            publish(&client, 0x02, 0x01, pm10);
            publish(&client, 0x02, 0x02, pm25);
